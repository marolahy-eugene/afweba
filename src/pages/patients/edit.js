...
import { FiSave, FiUser, FiCalendar, FiHome, FiBriefcase, FiLoader } from 'react-icons/fi';
...
export default function EditPatientPage() {
  ...
  useEffect(() => {
    const loadPatient = async () => {
      ...
      try {
        ...
        if (patientData) {
          // Formatter la date pour l'input type="date"
          let formattedDate = '';
          if (patientData.dateDeNaissance) {
            if (typeof patientData.dateDeNaissance === 'object' && 'seconds' in patientData.dateDeNaissance) {
              const date = new Date(patientData.dateDeNaissance.seconds * 1000);
              formattedDate = date.toISOString().split('T')[0]; // Format YYYY-MM-DD
            } else if (typeof patientData.dateDeNaissance === 'string') {
              formattedDate = patientData.dateDeNaissance;
            }
          }
        ...
        setSubmitError('Impossible de charger les données du patient.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      try {
        // Mettre à jour le patient dans Firebase
        await firebaseService.updatePatient(id, formData);
        ...
        setIsSubmitting(false);
        setSubmitError('Erreur lors de la mise à jour du patient. Veuillez réessayer.');
      }
    } else {
  ...
  return (
    <Layout>
      <div className="py-6">
        <div className="bg-custom-2 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-semibold text-primary">Modifier un patient</h1>
              ...
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm text-green-800">
                    Patient mis à jour avec succès ! Redirection en cours...
                  </p>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="mt-8 bg-card p-6 rounded-lg shadow-md">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  className={`mt-1 block w-full border rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white ${
                    errors.prenom ? 'border-red-300 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'
                  }`}
                />
                {errors.prenom && (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  className={`mt-1 block w-full border rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white ${
                    errors.nom ? 'border-red-300 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'
                  }`}
                />
                {errors.nom && (
                ...
                <div className="relative mt-1">
                  ...
                  <input
                    ...
                    className={`pl-10 block w-full border rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm ${
                      errors.dateDeNaissance ? 'border-red-300 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'
                    }`}
                  />
                </div>
                {errors.dateDeNaissance && (
                  <p className="mt-1 text-sm text-red-600 dark:text-red-400">{errors.dateDeNaissance}</p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={handleChange}
                  className={`mt-1 block w-full border rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white ${
                    errors.genre ? 'border-red-300 dark:border-red-500' : 'border-gray-300 dark:border-gray-600'
                  }`}
                >
            ...
            <div className="mt-8 border-t border-gray-200 pt-6 flex justify-end">
              <button
                ...
                className="mr-3 px-4 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary"
              >
                Annuler
              </button>
              <button
